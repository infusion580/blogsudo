...
function AdminEvents() {
  ...
  const load = () => {
    ...
    supabase
      .from("events")
      .select("id,slug,title,status,event_date,location")
  ...
  const toggleStatus = async (r: Row) => {
    ...
    const { error } = await supabase.from("events").update({ status: next }).eq("id", r.id);
    if (error) toast.error(error.message);
    else { toast.success(next === "published" ? "Publicado" : "Despublicado"); load(); }
  };
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-border/60 bg-card/30 p-16 text-center">
          <p className="text-muted-foreground">Aún no hay eventos. Creá el primero.</p>
        </div>
      ) : (
              ...
              {rows.map((r) => (
                <tr key={r.id} className="border-b border-border/40 last:border-0">
                  ...
                  <td className="px-4 py-3 text-sm">
                    {new Date(r.event_date).toLocaleDateString("es-ES", { day: "2-digit", month: "short", year: "numeric" })}
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="inline-flex gap-1">
                      <Button size="icon" variant="ghost" onClick={() => toggleStatus(r)}>
                        {r.status === "published" ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                      </Button>
                      <Link to="/admin/eventos/$id" params={{ id: r.id }}>
                        <Button size="icon" variant="ghost"><Pencil className="h-4 w-4" /></Button>
                      </Link>
                      <Button size="icon" variant="ghost" onClick={() => remove(r.id, r.title)} className="text-destructive hover:text-destructive">
